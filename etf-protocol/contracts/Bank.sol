...
contract Bank is ERC20, Ownable, ReentrancyGuard, Pausable {
    ...
    function deposit(
        ...
        address coin,
        uint256 amount
    ) external onlyEOA nonReentrant whenNotPaused returns (uint256) {
        (uint256 share, uint256[] memory amounts) = calShare(
            index,
            coin,
            amount
        );
    ...
    function calShare(
        ...
        for (uint256 i = 0; i < count; i++) {
            amounts[i] =
                (amount * ratios[i] * assetDecimals[assets[i]]) /
                assetDecimals[coin] /
                PRECISION;
            value = value + (amounts[i] * prices[i]) / (10 ** decimals[i]);
        }
        return (
            totalSupply() > 0 ? (sharePrePrice() * value) / PRECISION : value,
            amounts
        );
    }

    /*
     *@dev Calculate share Price
     */
    ...
    function updateRebalanced() internal {
        if (isRebalanced) {
            ...
            uint256[] memory deltaAmounts = Formula.calDeltaAmounts(
                currentRatios,
                targetRatios,
                amounts
            );
            ...
            for (uint256 i = 0; i < count; i++) {
                ...
                mistake += (deltaAmounts[i] * FACTOR) / newAmount <= DEVIATION
                    ? 0
                    : 1;
                if (mistake == 0) {
    ...
    /*
     *@dev Calculate transaction fees and mint
     */
    ...
    function withdraw(
        ...
        (address[] memory coins, uint256[] memory amounts) = calDecreaseCoins(
            share
        );
        calFee();
    ...
    function calGapCoin()
        internal
        view
        returns (uint256[] memory, uint256, address)
    {
        if (isRebalanced) {
            uint256[] memory amounts = getPoolAmounts();
            (uint256[] memory prices, uint8[] memory decimals) = getPrices();
            (uint256[] memory amt, uint256 index) = Formula.calMaxGapCoin(
                currentRatios,
                targetRatios,
                amounts,
                prices,
    ...
    function calIncreaseCoins(
        ...
        if (isRebalanced) {
            ...
            uint256[] memory amounts = getPoolAmounts();
            uint256[] memory increaseAmts = Formula.calIncrease(
                amounts,
                targetRatios,
                deltaAmounts,
                index,
    ...
    /*
     *@dev calculate reduced tokens
     */
    function calDecreaseCoins(
        uint256 share
        ...
        if (flag) {
            ...
            for (uint256 i = 0; i < count; i++) {
                coins[i] = assets[i];
                amounts[i] =
                    (IERC20(assets[i]).balanceOf(address(this)) * share) /
                    total;
            }
        }
